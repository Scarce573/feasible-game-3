...
class State(object):
	...
	def to_dict(self):
		...
		#state["pause_coag"] = self.pause_coag.to_dict() # *** NOTE: Don't need to save
	...
	def __init__(self, size, grid):
		...
		for x_val in range(len(grid)):
			...
			for y_val in range(len(saved_col)):
				...
				col.append(load_if_dict(saved_tile, [x_val, y_val]))
...
class Tile(object):
	...
	Tile.__init__(self, coords, layers=[])
	Tile.__repr__(self)
	Tile.__str__(self)
	Tile.get_mobs(self)
	Tile.to_dict(self)
	...
	def __init__(self, coords, layers=[]):
		"""
		Initialize the Tile.

		Note that coords does not come from the saved state, so it has to be the first argument.
		"""
		...
		for saved_layer_dict in layers:
			...
			for perm_str in saved_layer_dict:
				...
				layer[permeability] = load_if_dict(saved_layer_dict[perm_str], entity_coords)
...
class Entity(object):
	...
	Entity.__init__(self, coords, id_, inventory, knowledge, status)
	...
	def __init__(self, coords, id_, inventory, knowledge, status):
		"""
...
class NonMob(Entity):
	...
	NonMob.__init__(self, coords, id_, inventory, knowledge, status)
	NonMob.to_dict(self)
	...
	def __init__(self, coords, id_, inventory, knowledge, status):
		...
		super(NonMob, self).__init__(coords, id_, inventory, knowledge, status)
...
class Mob(Entity):
	...
	Mob.__init__(self, coords, id_, inventory, knowledge, status, ai, next_turn=None)
	...
	def __init__(self, coords, id_, inventory, knowledge, status, ai, next_turn=None):
		"""Initialize the Mob."""

		super(Mob, self).__init__(coords, id_, inventory, knowledge, status)
...
class Coagulate(object):
	...
	def __init__(self, name="", tree=[], method=[Game.co_pass], is_root=False):
		...
		# Fix any dictionary Coagulates
		for coag in tree:

			try: self._tree.append(load_if_dict(coag))
			except: exit(coag)

		# Construct method
		self.method = []

		# Escape the string forms of Game.* functions
		for arg in method:
			if type(arg) == str:
				if arg.find('$') != -1:
...
class Differentia(Coagulate):
	...
	Differentia.__init__(self, name="", id_="", tree=[], method=[Game.co_pass], is_root=False)
	...
	def __init__(self, name="", id_="", tree=[], method=[Game.co_pass], is_root=False):
		"""Initialize the Differentia."""

		super(Differentia, self).__init__(name, tree, method, is_root)
...
class Figment(Differentia):
	...
	Figment.__init__(	self, name="", id_="", actions=[], qualita_inate=[], qualita_inherited=[], 
						quanta_inate=[], quanta_inherited=[], is_root=False)
	...
	def __init__(	self, name="", id_="", actions=[], qualita_inate=[], qualita_inherited=[], 
		...
		super(Figment, self).__init__(name, id_, [], [Game.co_pass], is_root)

		# Inate quanta and qualita should have already been loaded from _tree *** DEBUG *** what did this mean?	
...
class Characteristic(Differentia):
	...
	Characteristic.__init__(self, name="", id_="", value=None, is_root=False)
	Characteristic.to_dict(self)

	Characteristic._value
	"""

	def __init__(self, name="", id_="", value=None, is_root=False):
		"""Initialze the Characteristic."""

		super(Characteristic, self).__init__(name, id_, [], [Game.co_pass], is_root)
...
class Qualita(Characteristic):
	...
	Qualita.__init__(self, name="", id_="", value=True, is_root=False)
	Qualita.to_dict(self)
	"""

	def __init__(self, name="", id_="", value=True, is_root=False):
		"""Initialze the Qualita, perhaps from a saved state."""

		super(Qualita, self).__init__(name, id_, value, is_root)
...
class Quanta(Characteristic):
	...
	def __init__(self, name="", id_="", value=0, is_root=False):
		"""Initialze the Quanta."""

		super(Quanta, self).__init__(name, id_, value, is_root)
...
class Action(Differentia):
	...
	Action.__init__(self, name="", id_="", qualita=[], quanta=[], meth_args=[[]], is_root=False)
	Action.to_dict(self)
	...
	def __init__(self, name="", id_="", qualita=[], quanta=[], meth_args=[[]], is_root=False):
		"""Initialze the Action, perhaps from a saved state."""

		super(Action, self).__init__(name, id_, [], [Game.co_pass], is_root)

		# *** NOTE *** Action is missing a tree and meth_args is pointless?
...
def load_if_dict(saved_state, *args):
	"""
	Load an instance from a dict with the "_type" key, if it's a dict

	Should this use copy.copy()?
	Meanwhile while I'm not using copy.copy(), it's __init__'s job to not damage saved_state.
	"""

	# Return saved_state if it isn't a dict to be loaded
	if type(saved_state) != dict:

		return saved_state

	# Make a copy and remove the _type key
	saved_state_copy = copy.copy(saved_state)
	type_ = saved_state_copy.pop("_type")

	return globals()[type_](*args, **saved_state_copy)